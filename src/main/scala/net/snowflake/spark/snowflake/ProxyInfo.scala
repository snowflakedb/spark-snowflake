...
private[snowflake] class ProxyInfo(proxyHost: Option[String],
  ...
  private def validate() : Unit = {
    ...
    if (proxyPassword.isDefined && !proxyUser.isDefined) {
      throw new IllegalArgumentException(
  ...
  def setProxyForJDBC(jdbcProperties : Properties) : Unit = {
    ...
    jdbcProperties.put(SFSessionProperty.PROXY_HOST.getPropertyKey(), proxyHost.get)
    jdbcProperties.put(SFSessionProperty.PROXY_PORT.getPropertyKey(), proxyPort.get)
    ...
    proxyUser match {
      case Some(optionValue) =>
        jdbcProperties.put(SFSessionProperty.PROXY_USER.getPropertyKey(), optionValue)
      case None =>
    }
    proxyPassword match {
      case Some(optionValue) =>
        jdbcProperties.put(SFSessionProperty.PROXY_PASSWORD.getPropertyKey(), optionValue)
      case None =>
    }
    nonProxyHosts match {
      case Some(optionValue) =>
        jdbcProperties.put(SFSessionProperty.NON_PROXY_HOSTS.getPropertyKey(), optionValue)
      case None =>
    }
  }
