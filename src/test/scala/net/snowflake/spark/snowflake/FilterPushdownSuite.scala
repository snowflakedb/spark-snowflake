...
class FilterPushdownSuite extends FunSuite {
  ...
  test("buildWhereClause with empty list of filters") {
    assert(buildWhereStatement(StructType(Nil), Seq.empty).toString === "")
  }
  ...
  test("buildWhereClause with multiple filters (keep column case)") {
    val filters = Seq(
      EqualTo("test_bool", true),
      // scalastyle:off
      EqualTo("test_string", "Unicode是樂趣"),
  ...
  test("buildWhereClause for different datatypes (keep column case)") {
    val filters = Seq(
      EqualTo("test_bool", true),
      // scalastyle:off
      EqualTo("test_string", "Unicode是樂趣"),
